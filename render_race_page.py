...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .wind-chart-wrap {
    ...
    margin: 4px 24px 24px; height: 280px; position: relative;
  }
  .wind-chart-wrap svg { width: 100%; height: 100%; display: block; }
...
<section id="wind-section" style="border-bottom: none">
...
(function () {
  ...
  /* ---------------- True Wind Speed over time (common to every N2K page) ---------------- */
  (function () {
    const chart = document.getElementById("windChart");
    const tooltip = document.getElementById("windTooltip");
    const chartWrap = chart.parentElement;
    ...
    function drawChart() {
      ...
      for (let i = 0; i <= ySteps; i++) {
        ...
        label.setAttribute("fill", "var(--dim)"); label.setAttribute("font-size", "11"); label.setAttribute("text-anchor", "end");
      ...
      for (let i = 0; i <= xSteps; i++) {
        const t = minT + ((maxT - minT) / xSteps) * i;
        const x = xScale(t);
        const line = document.createElementNS(svgNS, "line");
        ...
        for (const dpt of windData) { if (Math.abs(dpt.t - t) < Math.abs(nearest.t - t)) nearest = dpt; }
        const label = document.createElementNS(svgNS, "text");
        label.setAttribute("x", x); label.setAttribute("y", H - padB + 18);
        label.setAttribute("fill", "var(--dim)"); label.setAttribute("font-size", "11"); label.setAttribute("text-anchor", "middle");
        label.textContent = fmtLocalTs(nearest.ts);
        chart.appendChild(label);
      }

      const d = windData.map((p, i) => `${i === 0 ? "M" : "L"}${xScale(p.t).toFixed(1)},${yScale(p.tws).toFixed(1)}`).join(" ");
      const path = document.createElementNS(svgNS, "path");
      path.setAttribute("d", d);
      ...
      for (let i = 0; i < windData.length; i += HOVER_STEP) {
        const p = windData[i];
        const c = document.createElementNS(svgNS, "circle");
        c.setAttribute("cx", xScale(p.t)); c.setAttribute("cy", yScale(p.tws));
